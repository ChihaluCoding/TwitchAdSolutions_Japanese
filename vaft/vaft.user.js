...
(function() {
    ...
    // vaft のインスタンスになり、プレイヤーの制御を奪い合ってしまう。twitch.tv/CHANNEL でプレイヤーを持つのは
    ...
    'use strict';
    const ourTwitchAdSolutionsVersion = 90;// 古いバージョンのスクリプトとの競合を防ぐために使う
    ...
    function declareOptions(scope) {
        ...
        // 数週間の実地観測でセッション / ソースのメタデータであることが確認された DATERANGE のクラス。
        // そうでないものは「候補」として表に出す。候補のログからはこれらを除外し、診断が本当に新しい
        // マーカーに集中するようにしている。
        ...
        scope.FastAutoplayFirstTry = true;// 前回の広告で脱出処理により autoplay が採用された場合、次の広告では playerTypesToTry の先頭（位置 0）に autoplay（360p）を置く。4 つの Source タイプを順に試す（約 1.5〜2 秒）代わりに、最初の探索（約 340ms）で autoplay が採用される。Source 帯のタイプが採用された時点（チャンネルが回復した時点）で自動的に元に戻るため、Twitch が広告のない Source のバックアップを再び提供するようになれば画質は自動的に戻る。v67.1.0 以降はデフォルトで有効 — 実地のデータ（project_all_channels_csai_only_marked）により、観測されたすべてのチャンネルが広告のたびに Source 帯を使い切ることが確認されている。無効化は localStorage の twitchAdSolutions_fastAutoplayFirstTry=false による。
        ...
        scope.DisableAdSpoofing = true;// デフォルトで無効（v68.2.0 まではデフォルトで有効だった）。スプーフのビーコンが示す「常に 100% 視聴・音声あり・表示中」というパターン自体が異常として特徴づけられ、検出の強化（採用したバックアップにも CSAI が届く状態）を招く可能性がある。実地での相関が観測されており、TTV-AB のメンテナーも同じ仮説を立てている。スプーフが受理されたこと（GQL に拒否されなかったこと）は、特徴づけられていないことの証明にはならない。Twitch は 200 OK を返しつつ、そのビーコンのパターンを静かな検出の材料として使える。GQL の広告トラッキングのビーコン（video_ad_impression、video_ad_quartile_complete × 4、video_ad_pod_complete）を再び有効にするには twitchAdSolutions_disableAdSpoofing=false を設定してオプトインする。
        ...
        scope.EarlyReloadPollThreshold = 3;// 早期再読み込みを発動するまでの、全セグメント除去のポーリングの連続回数（1 回あたり約 2 秒なので 3 で約 6 秒、5 で約 10 秒、10 で約 20 秒。0 で無効）。testing 版に合わせて 5 から 3 に引き下げた — 実地の報告によれば、早期再読み込みを早めた方が SSAI の重い広告からきれいに復旧する。localStorage の twitchAdSolutions_earlyReloadPollThreshold で上書きできる。
        ...
        scope.PlayerBufferingDangerZone = 0.5;// 1 → 0.5 に引き下げた: ライブ端で薄いながらも機能しているバッファ（約 0.7〜0.95 秒）のとき、AND の判定（positionFrozen かつ bufferDuration < DangerZone）が実際の一瞬の停止で発動していた。しかしその「修正」である一時停止 / 再生は Twitch の再生モニターと相性が悪く（「バッファ済み領域 0.04xxx」に合わせ直される）、修正のたびに currentTime が悪化する自己増殖的な連鎖を生んでいた。しきい値を下げることで、介入が明らかに必要な、本当にバッファが尽きた状態（0.5 秒未満）に限定される。
    ...
    // すべてのフィールドの形状を最初に宣言した新しい StreamInfo を生成する。
    // streamInfo に新しいフィールドを追加するときは、全体像を 1 か所で把握できるよう、
    // 適切な初期値とともにここで宣言すること。
    function createStreamInfo(channelName, encodingsM3u8, usherParams) {
        return {
            ...
            SpoofedAdIds: new Set(),// notifyAdComplete: 今回の広告ですでにスプーフした stitched-ad の ID（複数ポーリングにまたがる重複排除）。広告の終了時にクリアされる。
            // 変更済み m3u8 の状態
            ModifiedM3U8: null,
            ...
            RecoverySegments: [],
            RecoveryStartSeq: undefined,// 重要: `!== undefined` により明示的に判定している
            FreezeStartedAt: 0,
            ConsecutiveAllStrippedPolls: 0,
            TotalAllStrippedPolls: 0,
            // 全セグメントが除去された場合の復旧に使う、クリーンなプレイリストのスナップショット（TTV-AB に準拠）
            ...
            // ハイブリッドモードの状態（PreferLowQualityBackup）
            EscapeHatchFired: false,
            LastBreakUsedEscapeHatch: false,// FastAutoplayFirstTry: PreferLowQualityBackup の脱出処理で autoplay が採用された広告（= その広告で 4 つの Source タイプがすべて汚染された場合）に設定される。次の広告でバックアップ探索に入るときに読まれ、autoplay を先頭に置く。Source 帯のタイプが採用された時点（チャンネルが回復した時点）でリセットされる。
            FastAutoplayConsecutive: 0,// Source 帯を試さずに高速な autoplay で処理できた広告の連続回数。定期的な再探索を発動し、チャンネルの回復（Twitch が CSAI 配信をやめた場合）を検出する。Source 帯の完全な探索を行うか、Source 帯が採用された時点でリセットされる。
            // 再読み込みのクールダウン
            LastPlayerReload: 0,
            ReloadTimestamps: [],
            // 診断用フラグ（セッションごとに 1 回）
    ...
    function getCleanWorker(worker) {
        ...
        while (proto) {
            ...
            if (workerStringConflicts.some((x) => workerString.includes(x))) {
                if (parent !== null) {
                    // 他の拡張機能が Worker.prototype を凍結していたり、[[Prototype]] を再設定不可にしていたりする
                    // 可能性がある。その場合 setPrototypeOf は TypeError を投げる。リンクごとに捕捉し、
                    // 1 つの凍結されたリングのためにチェーン全体の走査が中断されないようにする。
    ...
    function reinsertWorkers(worker, reinsert) {
        ...
        for (let i = 0; i < reinsert.length; i++) {
            // リンクごとの try-catch: 他の拡張機能が prototype のエントリを 1 つ凍結していても、
            // 再挿入のチェーン全体を壊すべきではない。失敗したリンクは飛ばして続行する。
                ...
                const newBlobStr = `
                    ...
                    self.addEventListener('message', function(e) {
                        if (e.data.key == 'UpdateClientVersion') {
                            ...
                            // これをクリアしないと、正常と判断してスキップした後もワーカーの EarlyReloadTriggered /
                            // EarlyReloadAwaitingResult が立ったままになり、後のポーリングが正当に再読み込みを求めても、
                            // その広告中の以降の早期再読み込みがブロックされてしまう。
                            let cleared = false;
                    ...
                    hookWorkerFetch();
                    // eval をガードする。不正な workerString によって、診断情報も出さずに Twitch のプレイヤーロジックが
                    // 壊れることがないようにする。例外が投げられてもワーカー自体は生き続けるが（vaft のフックは
                    // 上で設定済み）、Twitch 側のロジックは動かなくなる。
                    try { eval(workerString); } catch (e) { console.error('[AD DEBUG] ワーカーの eval に失敗 — Twitch のプレイヤーロジックが読み込まれていません:', e); }
                `;
                // ワーカーを置き換えるたびにメモリが積み上がらないよう、前の blob URL を revoke する
                            ...
                            if (response.status == 200) {
                                ...
                                if (streamInfo == null || streamInfo.EncodingsM3U8 == null) {
                                        ...
                                        if (lines[i].startsWith('#EXT-X-STREAM-INF') && lines[i + 1].includes('.m3u8')) {
                                            ...
                                            if (resolution) {
                                                const resolutionInfo = {
                                                    ...
                                                    // HEVC→AVC のフォールバック時には、書き換えた STREAM-INF の行に AUDIO / VIDEO / SUBTITLES の
                                                    // グループをコピーし、そのバリアントが対応するメディアグループを参照するようにする
                                                    // （TTV-AB v6.7.5 のパーサー修正に準拠）。これがないと、書き換えた行が元の HEVC バリアントの
                                                    // グループ ID を保持したままになり、AVC のバックアップには存在しないかもしれない音声トラックを
                                                    // 指してしまう。黒画面や音ズレの原因になる。
                                    ...
                                    if (AlwaysReloadPlayerOnAd || (decodableResolutionList.length > 0 && streamInfo.ResolutionList.some((element) => element.Codecs.startsWith('hev') || element.Codecs.startsWith('hvc') || element.Codecs.startsWith('av0')) && !SkipPlayerReloadOnHevc)) {
                                        // STREAM-INF の属性を置換または追加する（存在すれば置換、なければカンマの後に追加する）。
                                        // 下で、最も近い非 HEVC のバリアントから AUDIO / VIDEO / SUBTITLES のグループを書き換え後の
                                        // HEVC の行にコピーするために使う。TTV-AB v6.7.5 のパーサー修正に対応する。
                                            ...
                                            for (let i = 0; i < lines.length - 1; i++) {
                                                if (lines[i].startsWith('#EXT-X-STREAM-INF')) {
                                                    ...
                                                    if (resSettings[codecsKey].startsWith('hev') || resSettings[codecsKey].startsWith('hvc') || resSettings[codecsKey].startsWith('av0')) {
                                                        ...
                                                        // 最も近い非 HEVC のバリアントからメディアグループの属性をコピーし、書き換えた行が
                                                        // AVC のバックアップに実際に含まれる音声 / 映像 / 字幕のトラックを参照するようにする。
                                                        // TTV-AB v6.7.5 のパーサー修正。
    ...
    // 本来 Twitch のプレイヤーは広告の再生に伴って video_ad_impression / video_ad_quartile_complete /
    // video_ad_pod_complete のビーコンを送る。広告をブロックしているとこれらは一切送られない。
    // これを偽装することで「広告が正常に再生された」というシグナルを模倣し、検出の強化を
    // 抑えられる可能性がある。RADS トークンは stitched-ad の DATERANGE の行から抽出する。
    // 失敗は握りつぶす。通常の広告ブロックの流れを妨げないためである。
    function notifyAdComplete(textStr, streamInfo) {
        try {
            // ポッド内の各広告は、それぞれ固有の #EXT-X-DATERANGE:ID="stitched-ad-..." の行を持ち、
            // RADS トークン、ad-id、creative-id、line-item-id、position などもそれぞれ独自に持っている。
            // Twitch は各広告の DATERANGE をその広告が再生され始めるときにしか出さないため、6 本のポッドは
            // 広告の間、m3u8 のポーリング 1 回につき 1 本ずつ現れる。この関数は広告ありのポーリングのたびに
            // 呼ばれ、streamInfo.SpoofedAdIds がポーリングをまたいで重複を排除するので、各広告は現れた時点で
            // ちょうど 1 回だけスプーフされる（ポッド全体を N/N でカバーする）。
            ...
            // m3u8 の属性から得られる本当のポッドサイズを使い（各 DATERANGE に付いている）、なければ
            // 目に見える一致数にフォールバックする。広告が 1 回のポーリングにつき 1 本ずつ現れても、
            // ポッド内のすべての広告で total_ads の値が一貫するようにする。
            ...
            // ホットパスの早期打ち切り: notifyAdComplete は広告ありのポーリングのたびに実行されるようになったが、
            // 長い複数広告の広告では、ポッド全体をスプーフし終えた後にも数百回のポーリングが残る。
            // 重複排除の集合がポッド全体をカバーした後は、残りのポーリングはすべて無駄なので、
            // 一致ごとの parseAttributes のループの前に抜ける。（判定に使うサイズは今回のポーリングの追加前の
            // 値なので、ポッドを完成させるポーリング自体は実行され pod_complete も付く。短絡するのは
            // それ以降のポーリングだけである。）
            ...
            for (let i = 0; i < matches.length; i++) {
                // 重複排除の判定用の軽量な ID の事前抽出 — DATERANGE のキャプチャは必ず
                // ID="stitched-ad-<UUID>" で始まる。完全な parseAttributes() の前に重複排除の集合を
                // 調べることで、スプーフの段階でポーリングのたびにスプーフ済みの広告の属性文字列を
                // 再解析せずに済む。
                ...
                // m3u8 に明示的なポッド内の位置がある場合はそれを優先する（X-TV-TWITCH-AD-POD-POSITION="2" など）。
                // 古い形式では反復のインデックスにフォールバックする。
                const adPosition = parseInt(attr['X-TV-TWITCH-AD-POD-POSITION'] || String(i), 10);
                // こちらが主張するイベントと内部的に矛盾しないペイロードにする。quartile_complete{4} と
                // pod_complete を送ることは「広告を最後まで 100% 視聴した」という意味になる。それを
                // mute=true / volume=0 / visible=false / duration=0 と組み合わせれば、Twitch が検証したときに
                // 明らかな手がかりになってしまう。通常の視聴者の状態を模倣する: 音声オン、表示中、全尺。
                // 再生時間は m3u8 の属性があればそこから取る（なければ 0 とし、従来の挙動を保つ）。
                ...
                // この広告の 6 つのイベントを 1 回の GQL POST にまとめる。Twitch のエンドポイントは
                // JSON 配列によるバッチ処理をネイティブにサポートしている。リクエスト数が 6 分の 1 になり、
                // 6 本の個別リクエストを立て続けに投げるという、いかにもボット的な特徴も避けられる。
                ...
                // 下のポッド完了のサイズ判定に反映されるよう、バッチを組み立てる前にこの広告を
                // スプーフ済みとしてマークする。
                ...
                // pod_complete はポッドにつき 1 回だけ送る（広告ごとではない）。実際のプレイヤーはポッド全体が
                // 終わった後に pod_complete を 1 回だけ送る。広告ごとに送るのは（6 本のポッドなら 6 回）、
                // それ自体が特徴的な痕跡になる。重複排除の集合が本当のポッドサイズに達した広告（最後の広告）に
                // 付随させる。ポッドが最後まで現れなかった場合（一部の DATERANGE を取り逃した、広告が途中で
                // 終わったなど）、pod_complete は正しく送られない。実際のプレイヤーも到達していないポッドの完了を
                // 主張しないからである。防御的なフォールバック（重複排除の集合がない場合）としては、
                // シグナルを失わないよう広告ごとに pod_complete を送る。
                if (!spoofedSet || spoofedSet.size === podLength) {
                    batch.push(makePacket('video_ad_pod_complete'));
                    podCompleteSent = true;
                }
                // GQL のレスポンスステータスを監視し、「スプーフが受理された」場合と「スプーフを Twitch に
                // 拒否された」場合（400 / 403 / 429 / 5xx）を区別する。.catch が発火するのはネットワークエラーや
                // タイムアウトのときだけで、200 以外のステータスは正常に解決される。これがないと、検出の強化
                // （Twitch がスプーフのビーコンを拒否し始めること）が、実地のログからは見えない静かな失敗に
                // なってしまう。セッションごとに 1 回のガードにより、広告が連続してレート制限がかかっても
                // ログが氾濫しないようにしている。
            ...
            if (newSpoofed > 0) {
                ...
                // src= はスプーフした DATERANGE がどのストリーム由来か（メインか、採用されたバックアップの
                // プレイヤータイプか）を示す。これにより、ストリーム切り替えによる広告 ID の混在という制約が
                // 可視化される。メインとバックアップにまたがってスプーフしたポッドでは、途中で src が変わる。
                // pod-complete= は、今回のポーリングで唯一の video_ad_pod_complete を付けたかどうかを示す
                // （合計とポッドの比から推測するのではなく、ポッドの完了を明示的に確認できる）。
    ...
    function stripAdSegments(textStr, stripAllSegments, streamInfo) {
        ...
        if (!streamInfo.LastAdCachePruneAt || now - streamInfo.LastAdCachePruneAt > 60000) {
            ...
            // 長時間のセッションで際限なく増えないようキャッシュに上限を設ける。各エントリは URL の文字列と
            // Date.now() のタイムスタンプ（約 200〜300 バイト）で、広告が頻繁に入る数時間のセッションでは
            // Map が MB 規模に達しうる。サイズが 1000 を超えたら古い方から 200 件を削除する
            // （Map の反復順序は挿入順なので FIFO になる）。古い広告 URL が再び要求されることはまずないが、
            // もし要求されても、通常どおり除去の経路で再びキャッシュされる。
    ...
    async function processM3U8(url, textStr, realFetch) {
        ...
        if (haveAdTags) {
            ...
            // 各広告の DATERANGE をその広告が始まるときにしか開示しないため、複数広告のポッドでは
            // 1 回のポーリングにつき 1 本ずつ現れる。notifyAdComplete が SpoofedAdIds で重複を排除するので、
            // 各広告はその広告の間に 1 回だけスプーフされる（N/N の完全なカバー）。
            ...
            // 広告後の再読み込みループのガード: 広告の終了時に IsUsingModifiedM3U8 は false に戻される。
            // その後 8 秒ほどの間に広告の続きを示すマーカーが届くと、!IsUsingModifiedM3U8 の条件が満たされるため、
            // 次の広告検出で HEVC の再読み込みが再び発動してしまう。その結果、直前の再読み込みが落ち着いた
            // 数秒後に、余分なプレイヤーの破棄と再構築が起きる。直近に再読み込みしたばかりならこれを
            // スキップし、この広告はバックアップストリームの経路に任せる。
            ...
            // スティッキー CSAI の脱出処理（PreferLowQualityBackup）: スティッキー経路で全セグメント除去の
            // 状態が長く（約 8 秒）続いた場合は、バックアップ探索に移行する。復旧のループに座り込む代わりに
            // Source のバックアップと autoplay のフォールバックを試すことで、SSAI の重いチャンネルに
            // スティッキーなフリーズからの出口を与える。
            ...
            if (streamInfo.CsaiOnlyThisBreak && !streamInfo.IsUsingModifiedM3U8) {
                ...
                // 長時間フリーズした場合の早期再読み込み — 通常のバックアップ探索の経路（約 952 行目）にある
                // 判定と同じもので、スティッキー経路から早期に return するとこれを完全に飛ばしてしまうため、
                // ここにも置いている。これがないと、CSAI と確認済みのストリームで SSAI の重い広告が来たとき、
                // プレイヤーは広告の間ずっと薄い復旧キャッシュを再生し続けることになる（観測例: ポッド 1 本の
                // 広告で 35.9 秒のフリーズ、全セグメント除去のポーリングが 3 回、復旧キャッシュはセグメント 1 個）。
                // ポッド内の広告 1 本につき maxEarlyReloads までに制限しているため、再読み込みのループは起こり得ない。
                // 前回のポーリングでの早期再読み込みの結果を確認する（スティッキー経路は通常経路の判定より前に return するため）
            ...
            // PreferLowQualityBackup が有効な場合は、最終手段のフォールバックとして 'autoplay'（360p）を
            // 末尾に追加する。これは pixeltris の元々の挙動に準拠している。まず Source のバックアップを試し、
            // autoplay が使われるのはそれらがすべて広告入りだった場合だけである。利用できるときは Source の
            // 画質を保ちつつ、SSAI の重い広告でもクリーンなバックアップを確保できる。
            ...
            // FastAutoplayFirstTry: 直前の広告で PreferLowQualityBackup の脱出処理により autoplay が
            // 採用された場合（4 つの Source タイプがすべて汚染されていた場合）、そのチャンネルは現在
            // SSAI 一様である。Source 帯を探ると約 1.5 秒のバッファリングを無駄にするので、代わりに
            // autoplay を先に試す。今回 autoplay も汚染されていれば、反復は通常どおり playerTypesToTry の
            // 残りに進む。Source 帯のタイプが採用された場合（チャンネルが回復した場合）、下の
            // 「Source を採用」の分岐が LastBreakUsedEscapeHatch をリセットし、次の広告では通常の反復に戻る。
            // v67.1.0 以降はデフォルトで有効。twitchAdSolutions_fastAutoplayFirstTry=false で無効化できる。
            //
            // 定期的な再探索: 高速な autoplay が連続で採用された回数が N の倍数になるたびに、Source 帯の
            // 完全な探索を強制し、チャンネルの回復（Twitch が全面的な CSAI 配信をやめた場合など）を検出する。
            // これがないと、いったん高速 autoplay に入ったチャンネルはセッションの間ずっと 360p のままになり、
            // Source 帯が再テストされないため Source がクリーンな広告を取り逃してしまう。コストは N 回の広告に
            // 1 度、約 1.5〜2 秒の探索ループである。N=5 なら、通常の広告の頻度で再探索の間隔は約 25〜50 分になる。
            if (FastAutoplayFirstTry && streamInfo.LastBreakUsedEscapeHatch && PreferLowQualityBackup) {
                ...
                if (consecutive >= FastAutoplayReprobeInterval) {
                    // 再探索: 今回の広告では autoplay の並べ替えを行わず、Source 帯をテストさせる。カウンターは
                    // リセットする。Source 帯が依然としてすべて広告入りなら、下の autoplay 採用の経路が再び
                    // LastBreakUsedEscapeHatch=true を設定し、このサイクルが再開する。Source 帯が採用されれば
                    // LastBreakUsedEscapeHatch はクリアされる。
            ...
            if (backupM3u8 && streamInfo.IsShowingAd) {
                ...
                if (streamInfo.ActiveBackupPlayerType != backupPlayerType) {
                    ...
                    // 'autoplay' は決して固定しない。PreferLowQualityBackup は playerTypesToTry の末尾に autoplay を
                    // 置いたままにし、すべての Source タイプが広告入りの場合に反復の最後の最終手段の分岐が
                    // それを採用できるようにしている。autoplay を固定すると先頭に移動してしまい、最終手段の分岐は
                    // 代わりに新たな末尾にある別の広告入りの Source タイプを採用してしまう。その結果、ユーザーは
                    // 意図した 360p のクリーンな autoplay のバックアップではなく、広告マーカー付きの Source 帯の
                    // バックアップを得ることになる（除去 + 復旧の経路に入り、フリーズしやすくなる）。
                    ...
                    if (streamInfo.EscapeHatchFired) {
                        ...
                        // FastAutoplayFirstTry: そのチャンネルを SSAI 一様とみなすのは、4 つの Source タイプをすべて試して
                        // すべて汚染されていた場合に限る。「sourceTried が 0」のケース（前回の広告から固定されている場合）は
                        // 新しい情報を加えない。
                        if (FastAutoplayFirstTry && sourceTried >= 4) {
                            streamInfo.LastBreakUsedEscapeHatch = true;
                            // 完全な探索を実行した直後 — 再探索のカウンターをリセットする（Source 帯を試したばかりのため）
                            streamInfo.FastAutoplayConsecutive = 0;
                        }
                    } else if (FastAutoplayFirstTry && backupPlayerType !== 'autoplay') {
                        // Source 帯のタイプがクリーンに採用された — チャンネルが回復した。次の広告で通常の反復に戻るよう
                        // SSAI 一様の判断材料をリセットする。Source 帯で成功したばかりなので、再探索のカウンターも
                        // あわせてリセットする。
            ...
            if (streamInfo.CleanPlaylistCount >= 3 || !hasLiveSegments || slowPathReady) {
                ...
                if (shouldReload) {
                    ...
                    streamInfo.IsUsingModifiedM3U8 = false;
                    streamInfo.LastPlayerReload = Date.now();
                    // 除去や m3u8 の変更によってバッファが汚れている可能性がある場合はハード再読み込みを行う。
                    // ソフト再読み込みは既存の MediaSource のバッファを保持するため、除去 / BLANK_MP4 / 復旧の
                    // 注入によるタイムスタンプの乱れがそのまま残り、時間とともに音ズレが積み重なってしまう。
        ...
        if (playerForMonitoringBuffering) {
            try {
                ...
                if (!player.core) {
                    ...
                    // player.core.state.position がまとめて更新されるため（PR #183 のコミットメッセージを参照）、
                    // video 要素がなめらかに進んでいる間も約 12 秒フリーズして見えることがある。下の停止判定では
                    // state.position と併用し、state.position がまとめて更新されるまでの間があるだけでは発動せず、
                    // 本当の停止（両方がフリーズ）でのみ発動するようにしている。
                    ...
                    if (position !== undefined && bufferedPosition !== undefined) {
                        ...
                        // <video> 要素が実際に再生しようとしていない間は、バッファ停止の判定を完全にスキップする。
                        // 対象となるのは次の 2 つの状態である:
                        //   readyState < 2（HAVE_CURRENT_DATA）— MSE の初期化、シークの実行中、再読み込み直後の読み込み。
                        //     ここでは currentTime も state.position も実際に 0 だが、これは初期化の状態であって停止ではない。
                        //     段階的に対処すると再読み込みのループになってしまう。
                        //   videoEl.paused === true — 外側のブロックはすでに player.isPaused()===false を条件としているが、
                        //     IVS のプレイヤーのラッパーと下位の <video>.paused は、再読み込みの破棄処理や自動再生ポリシーによる
                        //     ミュートの切り替えの間、短時間食い違うことがある。<video>.paused が true のときはプレイヤーが
                        //     再生しようとしていないので、「停止」という表現は適切ではない。
                        // カウンターは保持する（増やさず、リセットもしない）: 本当の停止の途中で一時的な初期化の落ち込みが
                        // 挟まっても、次の有効なポーリングから数え直しにならないようにする。
                        const playerNotActivelyPlaying = videoEl && (videoEl.readyState < 2 || videoEl.paused);
                        // FFZ の音声コンプレッサーは player.load() をラップし、読み込みのたびに新しい <video> 要素を
                        // 作成する（src/sites/shared/player.jsx の replaceVideoElement）。その後 Twitch の再生モニターは、
                        // バッファの再構築中に新しい要素を「バッファ済み領域 0.04xxx」に合わせる。この短い立ち上がりの間、
                        // currentTime は平坦になり、state.position も再読み込み後のリセット値のままになる。これはまさに
                        // positionFrozen のパターンである。要素の同一性で入れ替わりを検出し、新たな再読み込みと同様に扱う
                        // （カウンターをクリアし、次の有効なポーリングで猶予を与えるよう復旧フラグを立てる）。
                        ...
                        // 停止の判定: 位置のフリーズの判定には、state.position と video.currentTime の両方が
                        // 変化していないことを要求するようにした。currentTime が進んでいればプレイヤーは正常に
                        // 再生できており、こちらが「修正」すべきではない。この誤発動により、低遅延の状態や画質変更の
                        // 直後に、ユーザーから見て 8 秒周期の唐突な中断が起きていた。
    ...
    function doTwitchPlayerTask(isPausePlay, isReload, reloadKind) {
        ...
        if (isReload) {
            ...
            // ハード再読み込みの間は事前にミュートして、MediaSource の破棄時の音声ノイズを隠す。
            // 新しい MSE の初期化は不連続の境界をまたぐため、最初のフレームで耳障りなノイズが出る。
            // `canplay`（音声がデコード可能になった時点）で復元し、安全のため 1500ms で打ち切る。
            // ユーザーがすでにミュートしていた場合はスキップする（意図を尊重する）。下にある既存の
            // 3000ms の LS 復元タイマーが最終的な保険として働く。
            if (hardReload) {
                try {
                    ...
                    // Twitch が黙って再ミュートするパターンの強い手がかりである（v633 の診断ログで確認済み）。
                    // これがないと、離席していたユーザーが戻ったときに配信がミュートされたままになる（v635 での
                    // Tgod1991 の報告）。セッションの最初からミュートしているユーザーは vaftEverUnmuted=false と
                    // なるため最終防衛線は働かず、ミュートは尊重される。セッションの途中で意図的にミュートする
                    // ユーザーは twitchAdSolutions_recoverFromSilentMute=false で無効化できる。
                    ...
                    if (v && (wasInitiallyUnmuted || shouldRecover)) {
                        ...
                        // setSrc({isNewMediaPlayerInstance:true}) は <video> 要素を置き換えるため、元の `v` に付けた
                        // リスナーは発火しない。イベントは新しい要素で発火する。そこで document（キャプチャフェーズ）で
                        // 待ち受け、Twitch が新しい MediaSource をどの <video> に取り付けても捕捉できるようにする。
                        // イベントを 3 つ設定しているのは、Edge が `loadeddata` / `playing` を `canplay` とは独立に
                        // 発行するためで、そのいずれもが新しい要素のミュート解除の準備ができたことを示す十分な合図になる。
                        // 冪等な `done` のガードにより、最初に発火したものを採用する。
                        let done = false;
                        const restore = () => {
                            if (done) return;
                            done = true;
                        ...
                        setTimeout(restore, 4000);// Edge の初期化の遅さに余裕を持たせるため 2500ms → 4000ms に引き上げた — issue #200 の後続報告で、MSE の初期化が 2500ms を超えるハード再読み込みでミュート状態になることが分かったため。
                        // 最終防衛線: こちらの復元にもかかわらず 5500ms の時点でプレイヤーがミュートのままなら
                        // （Twitch が約 3000ms に行う LS からの復元は、再読み込み前に取得したミュート状態のスナップショットが
                        // true だった場合に再びミュートしうる）、もう一度ミュート解除を強制する。冪等であり、すでに
                        // ミュート解除済みなら何もしない。ユーザーが明示的にミュートした場合はスキップする
                        // （`weJustPaused` の経路はユーザーのミュート意図を保持する。ここで
                        // `playerBufferState.userPauseIntent` を見るのは安価な代用である。Twitch のプレイヤーのコードでは
                        // 一時停止とミュートが概念的に相関しているためである）。
                        setTimeout(() => {
                            try {
                                const cur = document.querySelector('video');
                                if (cur && cur.muted) {
            ...
            // 最終防衛線によるミュート解除がブロックされてしまう（issue #200 の後続対応）。既存の 2 秒の
            // pause リスナーのガードを再利用している。
            if (hardReload) {
                playerBufferState.weJustPaused = Date.now();
            }
    ...
    async function handleWorkerFetchRequest(fetchRequest) {
        // AbortController による 5 秒のタイムアウト。ワーカーは GQL のリクエスト（アクセストークンと
        // 広告スプーフのビーコン）にこの経路を使う。タイムアウトがないと、応答しない GQL エンドポイントが
        // ワーカーのバックアップ探索のループを無期限に（ブラウザの既定の約 30 秒まで）止めてしまい、
        // ネットワークの一瞬の不調が広告中の数秒のプレイヤーのフリーズに変わってしまう。5 秒は通常の
        // Twitch の GQL の応答（1 秒未満）より十分に長いが、最悪の待ち時間を抑えられる。AbortError は
        // 既存の catch と FailedBackupPlayerTypes のロックアウトに自然に流れる。
    ...
    try {
        ...
        if (lsFastAutoplay === 'false') {
            ...
            console.log('[AD DEBUG] FastAutoplayFirstTry が localStorage で無効化されています — 広告のたびに Source 帯を完全に探索します（autoplay の高速経路は使いません）');
        }
        ...
        if (lsDisableAdSpoofing === 'false') {
            ...
            console.log('[AD DEBUG] AdSpoofing が localStorage のオプトインで有効になっています — 広告の検出時に GQL の広告トラッキングのビーコンを送信します');
        }
    ...
    hookWindowWorker();
    hookFetch();
    // fetch を経由しない CSAI の広告リクエストを検出するため、XHR をフックする
