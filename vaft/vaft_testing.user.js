...
(function() {
    ...
    function declareOptions(scope) {
        ...
        scope.FastAutoplayFirstTry = true;// 前回の広告で Source 帯を使い切っていた場合、反復の先頭に autoplay（360p）を置く（約 1.5〜2 秒の探索ループを節約できる）。Source 帯が回復すれば自動的に元に戻る。v638 以降はデフォルトで有効（観測されたすべてのチャンネルが、CSAI のみなのにマーカーが付く状態だったため）。無効化: twitchAdSolutions_fastAutoplayFirstTry=false。
        ...
        scope.RecoverFromSilentMute = true;// issue #200: ハード再読み込みの際、そのセッション中に vaft がミュート解除していれば、Twitch による静かな再ミュートのパターンから復旧する。デフォルトで有効。無効にするには twitchAdSolutions_recoverFromSilentMute=false を設定する。
    ...
    function createStreamInfo(channelName, encodingsM3u8, usherParams) {
        return {
            ...
            LastBreakUsedEscapeHatch: false,// FastAutoplayFirstTry の判断材料 — PreferLowQualityBackup の脱出処理で autoplay が採用された広告で設定される。Source 帯が採用されたらリセットされる。
    ...
    function notifyAdComplete(textStr, streamInfo) {
        try {
            ...
            for (let i = 0; i < matches.length; i++) {
                ...
                // quartile_complete{4} と pod_complete（「最後まで 100% 視聴した」）と内部的に矛盾しない
                // ペイロードにする: 音声オン、表示中、全尺。
    ...
    function stripAdSegments(textStr, stripAllSegments, streamInfo) {
        ...
        if (!streamInfo.LastAdCachePruneAt || now - streamInfo.LastAdCachePruneAt > 60000) {
            ...
            // FIFO による上限 — サイズが 1000 を超えたら古い方から 200 件を削除する（Map の反復順序は挿入順である）。
            // 古い広告 URL が再び要求されることはまずないが、もし要求されても、通常どおり除去の経路で
            // 再びキャッシュされる。
    ...
    async function processM3U8(url, textStr, realFetch) {
        ...
        if (haveAdTags) {
            ...
            // 広告ありのポーリングのたびに広告完了をスプーフする。複数広告のポッドでは 1 回のポーリングにつき
            // 1 本ずつ現れ、notifyAdComplete が SpoofedAdIds で重複を排除する（N/N の完全なカバー）。
            ...
            // 高速な autoplay が連続で採用された回数が N の倍数になるたびに再探索し、チャンネルの回復
            // （Twitch が CSAI 配信をやめた場合）を検出する。
            ...
            if (backupM3u8 && streamInfo.IsShowingAd) {
                ...
                if (streamInfo.ActiveBackupPlayerType != backupPlayerType) {
                    ...
                    // 具体的には、'autoplay' を固定すると playerTypesToTry の末尾（意図した最終手段の位置）から
                    // 先頭に移動してしまい、反復の最後のフォールバックが autoplay のクリーンな 360p のバックアップでは
                    // なく、別の広告入りの Source タイプを採用してしまう。
            ...
            if (streamInfo.CleanPlaylistCount >= 3 || !hasLiveSegments || slowPathReady) {
                ...
                // クールダウンは、広告中に繰り返し発動しうるバッファ監視の再読み込みなど、連鎖リスクのある経路には
                // 引き続き適用される。
        ...
        if (playerForMonitoringBuffering) {
            try {
                ...
                if (!player.core) {
                    ...
                    // 実際の再生の進行については video.currentTime を正とする。state.position はまとめて更新されるため、
                    // video 要素がなめらかに進んでいる間も約 12 秒フリーズして見えることがある（PR #194 / issue #193）。
                    ...
                    if (position !== undefined && bufferedPosition !== undefined) {
                        ...
                        // <video> 要素が実際に再生しようとしていない間（readyState < 2 または一時停止中）は、バッファ停止の
                        // 判定をスキップする。これらは初期化や一時停止の状態であって、停止ではない。
    ...
    function doTwitchPlayerTask(isPausePlay, isReload, reloadKind) {
        ...
        if (isReload) {
            ...
            // ハード再読み込みの間は事前にミュートして MSE の破棄時の音声ノイズを隠す。復元は複数のイベントで行い
            // （canplay / playing / loadeddata のうち最初に発火したものを採用）、安全のため 4000ms の setTimeout を
            // 併用する。ユーザーがすでにミュートしている場合はスキップする。5500ms の最終防衛線による強制的な
            // ミュート解除が、Twitch が約 3000ms に行う LS 復元による再ミュートを捕捉する（ユーザーの一時停止意図は
            // 保持する）。issue #200 / PR #201 / PR #206。
            if (hardReload) {
                try {
                    const v = document.querySelector('video');
                    const wasInitiallyUnmuted = v && !v.muted;
                    // issue #200 の修正 v636: vaftEverUnmuted をセッション単位にした（v635 ではタイムスタンプによる
                    // 減衰方式だったが、離席するユーザーには厳しすぎた。Tgod1991 の 2026-05-12 のログでは、10 分の
                    // ウィンドウが切れた後にミュートが尊重されてしまっていた）。そのセッション中に vaft が一度でも
                    // ミュート解除していれば、以降のハード再読み込みでは経過時間にかかわらず Twitch の静かな
                    // 再ミュートから復旧する。セッションの最初からミュートしているユーザー（vaftEverUnmuted=false）は
                    // 尊重される。localStorage の twitchAdSolutions_recoverFromSilentMute=false で無効化できる。
