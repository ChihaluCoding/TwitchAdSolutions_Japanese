...
(function() {
    ...
    // ドラッグした際にプレビューがフリーズする原因になっていた。（TTV-AB v6.4.9 と同期。）
    {
        ...
        if (_clipHost === 'clips.twitch.tv' || /^\/[^/]+\/clip\/[^/]+/.test(_clipPath)) {
            console.log('[AD DEBUG] video-swap-new-testing をスキップしました — クリップエディタのページです（' + _clipHost + _clipPath + '）。');
            return;
        }
    }
    const ourTwitchAdSolutionsVersion = 619;// 古いバージョンのスクリプトとの競合を防ぐために使う
    console.log('[AD DEBUG] TwitchAdSolutions video-swap-new-testing v' + ourTwitchAdSolutionsVersion + ' を読み込んでいます');
    if (typeof window.twitchAdSolutionsVersion !== 'undefined' && window.twitchAdSolutionsVersion >= ourTwitchAdSolutionsVersion) {
        console.log('[AD DEBUG] 競合: video-swap-new-testing v' + ourTwitchAdSolutionsVersion + ' をスキップしました — 別のスクリプトがすでに有効です（v' + window.twitchAdSolutionsVersion + '）。重複するスクリプトを削除してください。');
        return;
    }
    ...
    function stripAdSegments(textStr, stripAllSegments, streamInfo) {
        ...
        // （TTV-AB の LastCleanNativeM3U8 と同じ方式）。スナップショットが古いか存在しない場合は
        ...
        if (hasStrippedAdSegments && liveSegments.length === 0) {
            ...
            // それ以外は EarlyReloadPollThreshold 回のポーリング（約 10 秒）を待つ。薄い場合の回数上限は 2 である。
            ...
            if (EarlyReloadPollThreshold > 0 && streamInfo.ConsecutiveAllStrippedPolls >= effectiveThreshold && !streamInfo.EarlyReloadTriggered && (streamInfo.EarlyReloadCount || 0) < maxEarlyReloads) {
                ...
                // kind: 'early' は、バッファ監視による再読み込みのために設けられたクールダウンの判定を迂回する。
    ...
    async function processM3U8(url, textStr, realFetch) {
        ...
        // TTV-AB の LastCleanNativeM3U8 の仕組みに準拠。
                ...
                if (streamM3u8 != null) {
                    if (!hasAdTags(streamM3u8) && SimulatedAdsDepth == 0) {
                        ...
                        if (streamInfo.CleanPlaylistCount >= 2 || !hasLiveSegments || slowPathReady) {
                            ...
                            console.log('メインストリームの広告が終了しました。' + (ReloadPlayerAfterAd ? 'メインストリームに戻るためプレイヤーを再読み込みします...' : '再生を再開します...'));
    ...
    function reloadTwitchPlayer(isPausePlay, reloadKind) {
        ...
        // 再読み込みのクールダウン — 早期再読み込みは判定を迂回するが記録は更新するため、
        // 短時間に 3 回以上の早期再読み込みがあれば 90 秒の自動延長が働く。
