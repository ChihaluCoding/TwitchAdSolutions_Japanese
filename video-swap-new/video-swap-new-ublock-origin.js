...
(function() {
    ...
    // uBO は条件に一致するすべてに注入する。その一つひとつが競合するインスタンスになり、
        ...
        if (streamInfo.BackupEncodings) {
            ...
            if (streamM3u8Response.status == 200) {
                ...
                if (streamM3u8 != null) {
                    if (!hasAdTags(streamM3u8) && SimulatedAdsDepth == 0) {
                        // TTV-AB v6.6.7 の #1 に準拠: メインストリームで最初にクリーンなポーリングを観測した時点の
                        // タイムスタンプを記録し、その後のポーリングが再び広告ありに戻っても、下の低速経路の最大待機による
                        // 判定が発火できるようにする。
                        ...
                        // TTV-AB v6.6.7 の #4 に準拠: 低速経路の最大待機による独立した判定を設け、マーカーの揺れによって
                        // プレイヤーがバックアップに張り付いたままにならないようにする。
                        ...
                        // TTV-AB v6.6.7 の #1 に準拠: 揺れに強いリセットにより、広告マーカーが短時間戻る程度では
                        // PendingAdEndAt を維持し、マーカーの揺れで CleanPlaylistCount が進まない場合でも、
                        // 低速経路の最大待機による判定が発火できるようにする。
            ...
            if (!hasNonLiveSegment) {
                if (!streamInfo.HasLoggedCsaiFastPath) {
                    ...
                    // 表現を変更した — 元の「all segments live, skipping backup search」は恒久的な状態のように
                    // 読めたが、実際には今回のポーリングについて述べているだけである。Twitch は広告の途中で
                    // SSAI（stitched な広告セグメント）に切り替えることがあり、その時点でバックアップ探索は
                    // 正当に発動する。その遷移は別途ログに出力される。
    ...
    function reloadTwitchPlayer(isPausePlay, reloadKind) {
        ...
        // 再読み込みのクールダウン — 早期再読み込み（ワーカーが発動する、キャッシュが薄い場合の高速経路）は
        // 判定を迂回するが記録は更新するため、短時間に 3 回以上の早期再読み込みがあれば、暴走するループを
        // 防ぐための 90 秒の延長が働く。
